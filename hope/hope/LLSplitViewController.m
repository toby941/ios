...
#define kOverlayWidth 88  //交叉不分的大小
#define kOverlayBufferWidth 0  //阻尼区间宽度
...
- (void)viewDidLoad
{
    ...
    if(_viewStatu == KLLSplitViewStatu_Normal)
	{
        ...
        _controllerHome.view.frame = self.view.bounds;//CGRectMake(0, 0, 320, 460);
...
-(void)pan:(UIPanGestureRecognizer *)pan_
{
	...
	else if(pan_.state == UIGestureRecognizerStateChanged)
	{
        ...
        //只允许向右一侧拖拽
        if(currentCenter.x>_controllerHome.view.center.x){
...
- (void)showViewController:(UIViewController *)viewController_  animated:(BOOL)animated
{
    ...
    CGRect frame_ = self.view.bounds;//CGRectMake(0, 0, 320, 460);
	frame_.origin.x = 320;
...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)context
{
	if([animationID isEqualToString:@"hide"])
	{
		...
		self.controllerHome.view.frame = CGRectMake(325, 0, 320, self.view.bounds.size.height);//460
		[self.view addSubview:_controllerHome.view];
